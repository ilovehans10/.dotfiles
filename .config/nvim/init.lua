...
vim.api.nvim_create_autocmd("BufEnter", { group = "myterm", command = "if getwininfo(gettabinfo()[tabpagenr()-1]['windows'][(winnr()-1)])[0]['terminal'] | setlocal nonumber norelativenumber signcolumn=no| startinsert | else | stopinsert | endif" })
...
require("lazy").setup({
    ...
    },
    {
        ...
        event = "VeryLazy",
        config = function()
            keyset("n", "[c", function() require("treesitter-context").go_to_context(vim.v.count1) end, { silent = true })
        end
    },
    {
